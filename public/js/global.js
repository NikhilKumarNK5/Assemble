    ...
    socket.on('loggedInUser', function(users) {
        ...
        for(var i=0; i<users.length; i++) {
            if(friend.indexOf(users[i].name > -1)) {
                ...
                var list = '<img src="https://via.placeholder.com/150" class="pull-left img-circle" style="width:50px; margin-right:10px;" /> <p>' +
                '<a id="val" href="/chat/'+ userName.replace(/ /g, "-") +'.'+ name.replace(/ /g, "-") +'"><h3 style="padding-top=15px; color=gray; font-size=8px;">'+'@'+users[i].name+'<span class="fa fa-circle online_friend"></span></h3></a></p>'
                ol.append(list);
            }
        }
